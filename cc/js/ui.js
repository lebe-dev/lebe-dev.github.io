export function renderResults(btcAmount, sats, spread, marketRate, rateAge, source) {
  ...
  const spreadClass = spread < 0 ? 'spread-negative' : (spread > 2 ? 'spread-warning' : '');
...
export function renderHistory(entries) {
  ...
  if (!entries || entries.length === 0) {
    historyListEl.innerHTML = '<div class="text-center text-gray-400 py-8">No saved calculations</div>';
    return;
  }
  ...
  historyListEl.innerHTML = entries.map(entry => {
    ...
    const spreadClass = entry.spread < 0 ? 'spread-negative' : (entry.spread > 2 ? 'spread-warning' : '');
    ...
    return `
      <div class="history-item" data-id="${entry.id}">
        <div class="flex justify-between items-start">
          <div class="flex-1">
            <div class="text-sm text-gray-400 mb-1">${dateStr} MSK</div>
            <div class="font-semibold">${entry.btcAmount.toFixed(8)} BTC</div>
            <div class="text-sm text-gray-400">(${Math.floor(entry.btcAmount * 100000000).toLocaleString('en-US')} sats)</div>
            <div class="text-sm mt-2">
              <div>GEL: ${entry.gel.toLocaleString('en-US')}</div>
              <div>Office: ${entry.officeRate.toLocaleString('en-US')} GEL</div>
              <div>Market: ${entry.marketRate.toLocaleString('en-US')} GEL (${entry.source})</div>
              <div class="${spreadClass}">Spread: ${spreadSign}${entry.spread.toFixed(2)}%</div>
            </div>
          </div>
          <button class="btn btn-sm btn-ghost delete-btn" data-id="${entry.id}">
