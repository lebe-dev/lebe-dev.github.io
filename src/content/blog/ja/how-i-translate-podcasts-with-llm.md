...
そこで私は、ポッドキャストのエピソードを文字起こしし、ついでに複雑な用語を解説してもらうためにLLMを使うことにしました。

## どのモデルを使うか

[字幕の翻訳](/subtitles/)と同じく、Medium Effort のトップクラスのモデルを使っています。2026年8月現在、どこのモデルか（AnthropicかOpenAIか）は問いません。
...
文字起こしでは、プロンプトに書かなくてもLLMがデフォルトでWhisperを選びます。ただし、より簡易なモデルの場合は念のためモデルを明示的に指定したほうがよいでしょう。
...
文字起こしと翻訳は[ポッドキャストのページ](/ja/podcasts/)で公開しています。
