...
Поэтому я решил использовать LLM для транскрибации выпусков подкаста и попутно расшифровывать буддийские термины.

## Модель для перевода

Как и для [перевода субтитров](/subtitles/), я использую топовую модель с Medium Effort, не важно чья эта модель (Anthropic или OpenAI), важен контекст, который мы для неё задаём.
...
Claude выберет Whisper для транскрибации по умолчанию, даже если не указывать в промпте.
